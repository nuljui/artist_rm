                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                    {artist.profiles.map((profile, idx) => (
                        <div key={idx} className="flex gap-2">
                            ...
                            <input
                                type="text"
                                className="w-1/4 px-3 py-2 border border-ink/20 rounded-lg text-sm bg-canvas text-ink"
                                placeholder="Handle"
                            ...
                            <input
                                type="text"
                                className="w-1/6 px-3 py-2 border border-ink/20 rounded-lg text-sm bg-canvas text-ink"
                                placeholder="Followers"
                                value={profile.followers || ''}
                                onChange={e => {
                                    const newProfiles = [...artist.profiles];
                                    newProfiles[idx] = { ...newProfiles[idx], followers: e.target.value };
                                    setArtist({ ...artist, profiles: newProfiles });
                                }}
                            />
                            <div className="relative flex-1">
                                <input
                                    type="text"
                                    className="w-full pl-3 pr-8 py-2 border border-ink/20 rounded-lg text-sm bg-canvas text-ink"
                                    placeholder="URL"
                                    value={profile.url}
                                    onChange={e => {
                                        const newProfiles = [...artist.profiles];
                                ...
                                {profile.url && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="absolute right-2 top-2.5 text-ink/40 hover:text-accent transition-colors"
                                        title="Open Link"
                                    >
