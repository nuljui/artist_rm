...
interface ArtistListProps {
  data: Artist[];
  config: SheetConfig;
  viewMode?: 'assigned' | 'unassigned' | 'list';
  onAdd: (c: Artist) => void;
  onUpdate: (c: Artist) => void;
}

export const ArtistList: React.FC<ArtistListProps> = ({ data, config, viewMode, onAdd, onUpdate }) => {
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6 border-b border-ink/5 pb-8">
        <div className="space-y-1">
          ...
          <p className="text-lg text-ink/60 font-light">
            {viewMode === 'unassigned' ? 'Review and assign new leads.' : 'Track engagement & manage relationships.'}
          </p>
        </div>
